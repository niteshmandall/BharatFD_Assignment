...
def multiple_faqs(db):
    ...
    """
    FAQ.objects.create(
        question_en="What is Django?", answer_en="Django is a web framework."
    )
    FAQ.objects.create(
        question_en="What is AI?", answer_en="AI stands for Artificial Intelligence."
    )
...
def test_get_translated_question(faq_instance):
    ...
    assert (
        faq_instance.get_translated_question("fr") == faq_instance.question_en
    )  # Fallback to English
...
def test_get_single_faq(api_client, faq_instance):
    ...
    assert response.status_code == status.HTTP_200_OK
    assert (
        response.json()["question_en"] == faq_instance.question_en
...
def test_get_faq_with_invalid_language(api_client, faq_instance):
    ...
    assert response.status_code == status.HTTP_200_OK
    assert (
        response.json()[0]["question"] == faq_instance.question_en
